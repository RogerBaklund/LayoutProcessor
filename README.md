...
The `!else` statement has no condition, the provided code block is executed only if the corresponding 
`!if` conditon and all `!elseif` conditions are false. Unlike `!if` and `!elseif` it allows a statement 
...
it takes an array expression followed by the keyword `as` and a variable assignment or a key/value assignment. 
...
as global by the PHP script which called the `LayoutProcessor` `run_script()` or `run_layout()` methods.
...
if comma is used as separator for the foobar layout then all of these calls are equivalent:
